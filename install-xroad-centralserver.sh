#!/bin/bash
# Nome do Script: install-xroad-centralserver.sh
...
# Descrição: Instalação automatizada do X-Road Central Server single-node com banco local e sistema de cache.
...
SCRIPT_DESC="Instalação automatizada do X-Road Central Server single-node com cache"
XROAD_ROLE="Central Server"
XROAD_PACKAGE="xroad-centralserver"
...
main() {
    ...
    echo
    log INFO "Acesse a interface web do Central Server: https://$(hostname -f):4000/"
    echo
    log INFO "Guia oficial: https://docs.x-road.global/Manuals/ig-cs_x-road_6_central_server_installation_guide.html"
    echo
    mostrar_status_cache
}
